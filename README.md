...
- **저장 방식 선택**: 폴더별 일괄 저장 또는 중요/일반 구별 저장
...
1. 실행하면 **저장 방식을 먼저 선택**합니다.
   ```
   ============================================
     저장 방식을 선택하세요
   ============================================
     1) 일괄  — 폴더별로만 분류해서 저장
     2) 구별  — 폴더 안을 '중요 / 일반'으로 나눠서 저장

   선택 (1/2):
   ```
2. 선택 후 Chromium 브라우저가 자동으로 열립니다.
3. **네이버에 로그인**합니다 (2단계 인증 포함).
4. 로그인 완료 후 스크립트가 자동으로 진행됩니다.
5. 완료 후 현재 디렉토리의 `naver_memos/` 폴더에 `.md` 파일이 저장됩니다.
